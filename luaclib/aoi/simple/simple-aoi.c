...
tile_layer(struct tile* tl, int layer) {
	return tl->headers[layer];
}

static inline void
tile_push(struct tile* tl, int layer, object_t* object) {
	link_list_t* list = tile_layer(tl, layer);
...
static inline void
tile_pop(struct tile* tl, int layer, object_t* object) {
	link_list_t* list = tile_layer(tl, layer);
	...
	for (z = bl.z; z <= tr.z; z++) {
		for (x = bl.x; x <= tr.x; x++) {
			...
			if (self->layer == LAYER_ITEM) {
				...
				list = tile_layer(tl, LAYER_MONSTER);
				forearch_list(list, self, FLAG_OTHER | FLAG_SELF, ctx->enter_func, ud);

				list = tile_layer(tl, LAYER_USER);
				forearch_list(list, self, FLAG_OTHER | FLAG_SELF, ctx->enter_func, ud);
			}
		}
	}
