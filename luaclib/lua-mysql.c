...
static int
lstmt_execute(lua_State* L) {
	// lstmt_t* lstmt = lua_touserdata(L, 1);
	// size_t count = mysql_stmt_param_count(lstmt->stmt);
	// if (count != lua_gettop(L) -1) {
	// 	luaL_error(L,"stmt execute error:params count mismatch");
	// }
	...
	// 			lstmt->params[i].buffer_type = MYSQL_TYPE_TINY;
	// 			lstmt->params[i].buffer = &b;
	// 			break;
	// 		}
	// 		case LUA_TNUMINT: {
	// 			int inter = lua_tointeger(L, index);
	// 			lstmt->params[i].buffer_type = MYSQL_TYPE_LONG;
	// 			lstmt->params[i].buffer = &inter;
	// 			break;
	// 		}
	// 		case LUA_TNUMFLT: {
	// 			double d = lua_tonumber(L, index);
	// 			lstmt->params[i].buffer_type = MYSQL_TYPE_DOUBLE;
	// 			lstmt->params[i].buffer = &d;
	// 			break;
	// 		}
	// 		case LUA_TSTRING: {
	// 			size_t size;
	// 			const char* str = lua_tolstring(L, index, &size);
	// 			lstmt->params[i].buffer_type = MYSQL_TYPE_STRING;
	// 		    lstmt->params[i].buffer = str;
	// 		    lstmt->params[i].buffer_length = size;
	// 		    break;
	// 		}
	// 		default: {
	// 			luaL_error(L, "bind type error:%s", lua_typename(L, vt));
	// 			break;
	// 		}
	// 	}
	// }
	...
	// 	luaL_error(L, "mysql_stmt_execute(), failed:%s\n", mysql_stmt_error(lstmt->stmt));
	// }
}


static int
lstmt_release(lua_State* L) {
	lstmt_t* lstmt = lua_touserdata(L, 1);
	mysql_stmt_close(lstmt->stmt);
	free(lstmt->values);
	return 0;
}
...
lstmt(lua_State* L) {
	...
	// if (stmt == NULL) {
	// 	luaL_error(L, "create mysql stmt error:%s", mysql_error(&mysql->conn));
	// }

	// if (mysql_stmt_prepare(stmt, sql, size)) {
	// 	luaL_error(L, "mysql stmt prepare error:%s", mysql_stmt_error(stmt));
	// }

	// lstmt_t* lstmt = lua_newuserdata(L,sizeof(*lstmt));
	// lstmt->stmt = stmt;

	// size_t count = mysql_stmt_param_count(stmt);
	// lstmt->params = malloc(count * sizeof(*lstmt->params));
	// lstmt->res = mysql_stmt_result_metadata(stmt);

	// count = mysql_num_fields(lstmt->res);
	// lstmt->values = malloc(col_count * sizeof(*lstmt->values));

	// if (luaL_newmetatable(L, "meta_mysql_stmt")) {
	// 	const luaL_Reg meta[] = {
	// 		{ "execute", lstmt_execute },
	// 		{ NULL, NULL },
	// 	};
	// 	luaL_newlib(L,meta);
	// 	lua_setfield(L, -2, "__index");

	// 	lua_pushcfunction(L, lstmt_release);
